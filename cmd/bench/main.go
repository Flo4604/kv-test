...
func runBench(ctx context.Context, args []string) error {
	...
	maxConns := fs.Int("max-conns", 20, "max pool size (sized for M-20: 50 conn cap, two pools)")
	output := fs.String("output", "bench_results.json", "machine-readable results path")
	concurrenciesStr := fs.String("concurrencies", "1,2,5,10", "comma-separated concurrency levels (sized for M-20: 0.25 vCPU primary)")
